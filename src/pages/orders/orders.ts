...
export class OrdersPage {
  ...
  constructor(
    ...
    this.showLast24Hr = true;
    this.showOlder = false;
    if(this.user.name == "Guest User"){
      console.log("need to sort orders array due to guest user");
      this.user.orders.sort(this.userService.sortOrdersByTimeEarliestFirst);
    }
    if(this.user.orders){
      for(var i=0; i<this.user.orders.length; i++){
        this.user.orders[i]['isTodaysOrder'] = this.isTodaysOrder(this.user.orders[i]);
        this.user.orders[i]['isOldOrder'] = this.isOldOrder(this.user.orders[i]);
      }
    }
    this.presentLoading();
  }
  ...
  isTodaysOrder(order){
    ...
    var timeDiff = this.now.getTime() - orderDate.getTime();
  ...
  flipShowLast24Hr(){
    this.showLast24Hr = !this.showLast24Hr;
  }
